...
When the user wants to change `trellis-research`, `trellis-implement`, `trellis-test`, or `trellis-check` behavior, edit platform agent files in the user project.
...
4. Current task `implement.jsonl` / `test.jsonl` / `check.jsonl`
...
| Test quality must be triaged before final review | `trellis-test` + `test.jsonl` configuration rules |
