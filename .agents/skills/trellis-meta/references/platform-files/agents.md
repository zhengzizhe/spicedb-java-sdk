...
| `trellis-test` | Triage PRD acceptance criteria and the current diff, add focused tests when useful, and report remaining test risk. |
...
- current task `prd.md`
- `info.md`
- `implement.jsonl`, `test.jsonl`, or `check.jsonl`
...
| Focused test triage should run for behavior changes | `trellis-test` agent file, workflow Phase 2.2, and testing specs if needed. |
