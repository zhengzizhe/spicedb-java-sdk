...
Every task has its own directory under `.trellis/tasks/{MM-DD-name}/` holding `prd.md`, `implement.jsonl`, `test.jsonl`, `check.jsonl`, optional `research/`, and `info.md`. Beads-backed tasks store identity/status in Beads and bind the folder with `.bead`; legacy local folders may still have `task.json`.
...
# Code-spec context (injected into implement/test/check agents via JSONL).
# `implement.jsonl` / `test.jsonl` / `check.jsonl` are seeded on `task create` for sub-agent-capable
...
- 2.2 Focused test triage `[conditional · repeatable]`
...
| Finished writing / want to verify | Run Phase 2.2 test triage when behavior changed, then dispatch `trellis-check` per Phase 2.3 |
...
| Finished writing / want to verify | Focused test triage when behavior changed, then `trellis-check` |
| Stuck / fixed same bug several times | `trellis-break-loop` |
| Spec needs update | `trellis-update-spec` |
...
| "Code first, check later" | Focused test triage and `trellis-check` surface real gaps while keeping the loop small |
...
Curate `implement.jsonl`, `test.jsonl`, and `check.jsonl` so the Phase 2 sub-agents get the right spec context. These files were seeded on `task create` with a single self-describing `_example` line; your job here is to fill in real entries.

**Location**: `{TASK_DIR}/implement.jsonl`, `{TASK_DIR}/test.jsonl`, and `{TASK_DIR}/check.jsonl` (already exist).
...
**Split between the three files**:
- `implement.jsonl` → specs + research the implement sub-agent needs to write code correctly
- `test.jsonl` → testing guidelines, quality specs, and research the test sub-agent needs to judge coverage
...
python3 ./.trellis/scripts/task.py add-context "$TASK_DIR" test "<path>" "<reason>"
...
| `test.jsonl` has testing/quality context when behavior or test coverage changes | recommended |
...
Use this step when the task changes behavior, touches tests, fixes a bug, or the user asks for stronger coverage. Skip it for docs-only edits, mechanical formatting, or implementation-only refactors where `trellis-check` can cover the risk.

Spawn the test sub-agent in focused mode:

- **Agent type**: `trellis-test`
- **Task description**: Triage PRD acceptance criteria and the current diff; add only focused tests that catch a real regression; run the narrowest relevant tests and report remaining test risk

The test agent's job:
- Decide whether the change needs new/stronger tests or only a risk note
- Add focused regression or boundary tests when behavior is clear
- Escalate to a fuller matrix only for public API, auth/permission, persistence, migration, concurrency, cross-service, repeated bug, or explicit quality-audit tasks
- Avoid Semgrep/Snyk/network-backed scans unless the user explicitly asks or a local configured command is clearly warranted
- Run focused tests and report any remaining untested risk

[/Claude Code, Cursor, OpenCode, Codex, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi]

[Kilo, Antigravity, Windsurf]

When behavior changed, do focused test triage in the main session before the quality check:
- Decide whether the diff needs new/stronger tests or only a risk note
- Add focused regression or boundary tests when behavior is clear
- Run focused tests and record remaining test risk
...
- `test` or `check` reveals a prd defect → return to Phase 1, fix `prd.md`, then redo 2.1
...
Flow: trellis-implement → focused test triage when warranted → trellis-check → trellis-update-spec → finish
Next required action: inspect conversation history + git status, then execute the next uncompleted step in that sequence.
For agent-capable platforms, the default is to dispatch `trellis-implement` for implementation, dispatch `trellis-test` only when Phase 2.2 applies, and always dispatch `trellis-check` before reporting completion — do not edit code in the main session by default.
Use the exact Trellis agent type names when spawning sub-agents: `trellis-implement`, `trellis-test`, `trellis-check`, or `trellis-research`. Generic/default/generalPurpose sub-agents do not receive `implement.jsonl` / `test.jsonl` / `check.jsonl` injection.
