...
public class DepthAnalysis {
    ...
    static void measure(AuthCsesClient client, String title, String path, String desc,
        ...
        var result = client.on(type).resource(id).check(perm)
                .withConsistency(com.authcses.sdk.model.Consistency.full()).by(user);

        // 热身
        for (int i = 0; i < WARMUP; i++) {
            client.check(type, id, perm, user,
                    com.authcses.sdk.model.Consistency.full());
        }
