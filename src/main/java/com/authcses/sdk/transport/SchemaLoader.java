...
public class SchemaLoader {
    ...
    /** Set to false once we detect UNIMPLEMENTED, avoiding repeated failed calls. Per-instance, not static. */
    private volatile boolean reflectSchemaSupported = true;
     ...
     */
    public boolean load(ManagedChannel channel, Metadata authMetadata, SchemaCache schemaCache) {
        if (!reflectSchemaSupported) return false;
        try {
            ...
            if (e.getStatus().getCode() == Status.Code.UNIMPLEMENTED) {
                reflectSchemaSupported = false;
                LOG.log(System.Logger.Level.INFO,
                        "ExperimentalReflectSchema not supported by this SpiceDB version, schema validation disabled");
                return false;
            }
