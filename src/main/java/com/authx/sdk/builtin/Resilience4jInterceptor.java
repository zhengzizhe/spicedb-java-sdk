...
public class Resilience4jInterceptor implements SdkInterceptor {
    ...
    private void acquirePermissions(OperationContext ctx) {
        ...
        if (rateLimiter != null) {
            try {
                ...
                throw new AuthxException("Rate limited: max requests/second exceeded", e);
            }
        }
        if (bulkhead != null) {
            if (!bulkhead.tryAcquirePermission()) {
                eventBus.publish(new SdkTypedEvent.BulkheadRejected(Instant.now(), ctx.action().name()));
                throw new AuthxException("Bulkhead rejected: max concurrent requests exceeded");
            }
    ...
    public static class Builder {
        ...
        public Builder rateLimiter(int maxPerSecond) {
            this.rateLimiter = RateLimiter.of("authx-sdk", RateLimiterConfig.custom()
        ...
        public Builder bulkhead(int maxConcurrent) {
            this.bulkhead = Bulkhead.of("authx-sdk", BulkheadConfig.custom()
