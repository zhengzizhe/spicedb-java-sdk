    ...
    void coalescing_exceptionSharedToAllWaiters() throws Exception {
        ...
        SdkTransport failingDelegate = new InMemoryTransport() {
            ...
            public CheckResult check(CheckRequest request) {
                ...
                throw new AuthCsesConnectionException("boom");
            }
        };
        ...
        for (var f : futures) {
            try {
                ...
                assertThat(e.getCause()).isInstanceOf(AuthCsesConnectionException.class);
                exceptionCount++;
            }
        }
    ...
    void retryBudget_limitsRetries_underLoad() throws Exception {
        ...
        for (int i = 0; i < requestCount; i++) {
            futures.add(executor.submit(() -> {
                ...
                try {
                    transport.check(sameRequest());
                } catch (AuthCsesConnectionException ignored) {
                    // expected
                }
            }));
        }
