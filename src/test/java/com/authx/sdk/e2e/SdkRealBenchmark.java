...
public class SdkRealBenchmark {
    ...
    static AuthxClient buildClient(boolean cacheEnabled) {
    ...
    // ---- Check benchmark ----
    static void runCheckBenchmark(String name, AuthxClient client, int threads, int opsPerThread,
    ...
    // ---- Write benchmark ----
    static void runWriteBenchmark(String name, AuthxClient client, int threads, int opsPerThread) throws Exception {
        int totalOps = threads * opsPerThread;
        long[] latencies = new long[totalOps];
        AtomicLong idx = new AtomicLong(0);
        AtomicLong errors = new AtomicLong(0);
    ...
    // ---- Lookup benchmark ----
    static void runLookupBenchmark(String name, AuthxClient client, int threads, int opsPerThread,
    ...
    static void seedData(AuthxClient client, int docCount, int userCount) {
    ...
    static void warmCache(AuthxClient client, int docCount, int userCount) {
        for (int d = 0; d < docCount; d++) {
            for (int u = 0; u < userCount; u++) {
    ...
    static void cleanupData(AuthxClient client, int docCount, int userCount) {
        for (int d = 0; d < docCount; d++) {
            for (int u = 0; u < userCount; u++) {
                if ((d + u) % 5 == 0) {
                    try {
