...
class HealthProbeTest {
    ...
    void probeResult_up_leaf_hasEmptyChildren() {
        HealthProbe.ProbeResult r = ProbeResult.up("foo", Duration.ofMillis(5), "ok");
    ...
    void probeResult_down_leaf_hasEmptyChildren() {
        HealthProbe.ProbeResult r = ProbeResult.down("foo", Duration.ofMillis(99), "boom");
    ...
    void probeResult_nullChildren_normalizedToEmptyImmutable() {
        HealthProbe.ProbeResult r = new ProbeResult("x", true, 1, "ok", null);
        assertThat(r.children()).isEmpty();
    ...
    void probeResult_subMillisecondLatency_preservedInNanos() {
        ...
        HealthProbe.ProbeResult r = ProbeResult.up("fast", Duration.ofNanos(500_000), "ok");  // 500 µs
    ...
    void probeResult_children_defensivelyCopied() {
        ArrayList<HealthProbe.ProbeResult> mutable = new ArrayList<ProbeResult>();
        mutable.add(ProbeResult.up("a", Duration.ZERO, "ok"));
        HealthProbe.ProbeResult r = new ProbeResult("root", true, 0, "", mutable);
        mutable.clear();
    ...
    void compositeAny_oneUp_reportsUp() {
        HealthProbe composite = HealthProbe.any(
    ...
    void downFactory_alwaysUnhealthy() {
        HealthProbe.ProbeResult r = HealthProbe.down("manual override").check();
        assertThat(r.healthy()).isFalse();
